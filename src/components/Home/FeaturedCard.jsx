...
const FeaturedCard = ({ eventImg, title, date, imgalt, id, description }) => {
  return (
    <motion.article className="grid" initial="initial" whileHover="hover">
      ...
      <div className="self-end text-white flex w-full justify-between bg-[var(--color-surface-highlight-primary)] p-2 z-100">
        <span className="text-white text-pretty">{title}</span>
        <span className="text-white text-right">{eventDate(date)}</span>
      </div>
    </motion.article>
  );
};
